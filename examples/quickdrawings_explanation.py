...
# Indices of support vectors
    ...
    idx_obj = randint(0, len(X_test) - 1)  # Ex-positiveMISV 3084 # 
...
# Contextualized evaluation process
    ...
    print (n_train_cols, test_coords[idx_obj], train_coords[idx_proMISV], train_coords[idx_conMISV])   
...
# h0.text(0, 0, 'Drawing:', fontsize=10, style='italic', fontweight='bold')
...
#   Explanation with 'E'xample, 'C'ounterexample, 'B'oth
    ...
    if explanationType == 'E':
        ...
        ax1.text(0, 0.5, 'should be {0} because it looks like '.format(class_name), wrap=True)
        ...
        ax2.imshow(misvPro, cmap=cm, interpolation='none')
        ax3.text(0, 0.5, ', which has been identified as {0}.'.format(class_name), wrap=True)
        ax4.text(0, 0.5, 'However, the drawing could not be {0} because it also looks like '.format(class_name), wrap=True)
        ax5.imshow(misvCon, cmap=cm, interpolation='none')
        ax6.text(0, 0.5, ', which has not been recognized as such.', wrap=True)


    plt.savefig("{0}/explaining_prediction-drawing_{1}.pdf".format(output_dir, idx_obj), dpi=300)
    plt.show()
