...
import os, sys
from common import  ordinalltx, ordinal, hog
import time
...
# cm = plt.cm.gray
    ...
    while idx_obj < len(test_coords):
        ...
        sh2.text(0.5, 0, 'Example', style='italic', weight='demibold', ha='center', size='medium')
        sh3.text(0.5, 0, 'Counterexample', style='italic', weight='demibold', ha='center', size='medium')
        ...
        plt.savefig("{0}/top_{2}_predictions-digit_{1}.pdf".format(output_dir, idx_obj, k), dpi=300)
        idx_obj+= 250
        plt.show()
